...
module axi_pixel_fifo # (
	...
	// FIFO RX and TX buffer signaling
	...
	task xfer_reset(); begin
		...
		fifo_start_xfer <= 0;          // Mark the FIFO as ready
	end endtask

	/* verilator lint_off INITIALDLY */
	task hard_reset(); begin
		xfer_reset();
		core_clock_en <= 0;            // The core clock shouldn't be enabled
	...
	always @(posedge s_axis_clock) begin
		...
		case (state)
			// ================================================
			//    Reset the display on power up to all black
			// ================================================
			...
			// ================================================
			//    Haadle / Prepare for AXI data transfer ops
			// ================================================
